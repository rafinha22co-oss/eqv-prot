...
/* ---------- barra superior ---------- */
export function Topbar({ phase, open, view, onView, onHome, onAuto, onReset, onTheme, auto }) {
    ...
    <header className="topbar">
      ...
      <div className="right">
        <div className="seg" role="tablist" aria-label="Visão">
          <button role="tab" aria-selected={view === 'cerebro'} className={view === 'cerebro' ? 'on' : ''} onClick={() => onView('cerebro')}>Cérebro</button>
          <button role="tab" aria-selected={view === 'canvas'} className={view === 'canvas' ? 'on' : ''} onClick={() => onView('canvas')}>Canvas</button>
        </div>
...
/* ---------- memória da campanha ---------- */
export function Memory({ decisions }) {
  return (
    <div className="mem">
      <div className="mem-head">
        <h3>Cada decisão tem uma origem.</h3>
        <p>O que foi dito, confirmado e ajustado durante a jornada.</p>
      </div>
      {decisions.length === 0
        ? <div className="empty" style={{ margin: 'var(--s16)' }}>Ainda não há decisões. Elas aparecem aqui conforme a conversa avança.</div>
        : <ol className="mem-list">
          {decisions.map((d, i) => (
            <li key={i}><b>{d.titulo}</b><p dangerouslySetInnerHTML={{ __html: d.valor }} /><small>{d.origem}</small></li>
          ))}
        </ol>}
    </div>
  )
